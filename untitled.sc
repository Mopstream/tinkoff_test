object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val requestQueue: ArrayBuffer[(Byte, VarUInt, VarUInt)] = ArrayBuffer((Commands.WhoIsHere, Timer.currentTime, broadcast))
    ...
    //while (true) {
    val request = basicRequest.body(requestBody).post(url)
    val response = client.send(request)

    (response.body, response.code) match {
      case (Right(body), Ok) => requestBody = handle(body)
      case (_, NoContent) => exit(0)
      case _ => exit(99)
    }
    //}
      ...
      for {
        ...
        request += (
          currPayload.cmd match {
            case Commands.WhoIsHere => whoIsHereHandler()
            case Commands.IAmHere => iAmHereHandler()
            case Commands.Status => statusHandler()
            case Commands.Tick => tickHandler()
            case _ => ""
          })
        ...
        def iAmHereHandler(): String = {
          if (requestQueue.exists(_._1 == Commands.WhoIsHere)) {
            ...
            if (currPayload.devType != Device.EnvSensor) requestQueue += ((Commands.GetStatus, Timer.currentTime, currPayload.src))
            Packet(Payload(myAddress, currPayload.src, mySerial ++, currPayload.devType, Commands.GetStatus, EmptyBody())).encode
          } else {
            forgetIt()
            ""
          }
        ...
        def statusHandler(): String = {
          currCmdBody match {
            case EnvSensorStatus(values) => manageSensor(values, namesMap(currPayload.src))
            case ValueBody(value) if currPayload.devType == Device.Switch => manageSwitch(value)
            case _ =>
              requestQueue.find(x => (x._3 == myAddress) && (x._1 == Commands.GetStatus || x._1 == Commands.SetStatus)) match {
        ...
        def tickHandler(): String = {
          currCmdBody match {
            case TimerCmdBody(timestamp) => Timer.currentTime = timestamp
          }
          ""
        }
        ...
        def rememberIt(): Unit = {
          currCmdBody match {
            case Device(currName, currProps) =>
              namesMap(currPayload.src) = currName
              devicesMap(currName) = (currPayload.src, currProps, currPayload.devType)
          }
        }
        ...
        def manageSwitch(value: Byte): String = {
          var mes = ""
          val props = devicesMap(namesMap(currPayload.src))._2
          props match {
  ...
  src = "IgP_fwgDAghTV0lUQ0gwMQMFREVWMDEFREVWMDIFREVWMDMo"
